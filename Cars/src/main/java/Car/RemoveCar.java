...
class RemoveCar implements ActionListener{
    JFrame f;
    JTextField t;
    ...
    RemoveCar(){
        ...
        l5.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("Cars/CarIcon/remove.jpg"));
        l5.setIcon(img);
        f.add(l5);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b){
            try{
                Conn con = new Conn();
                String str = "select make,engnum,engcap from employee where reg ='"+t.getText()+"' ";
                ResultSet rs = con.s.executeQuery(str);

                int i=0;
                if(rs.next()){
                    String make = rs.getString(1);
                    String engnum= rs.getString(2);
                    String engcap=rs.getString(3);
        ...
        if(ae.getSource()==b1){
            try{
                Conn con = new Conn();
                String str = "delete from project12 where reg = '"+t.getText()+"'";
